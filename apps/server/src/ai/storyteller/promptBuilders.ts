...
export const buildNarrateActionPrompt = (
  ...
  composePrompt(promptTemplates, "narrative_director", [
    ...
    "Reward player for good roleplay or smart actions with cool loot or social rewards.",
    ...
    "Fumble can mean a failure, but should not stop the narrative and nor end of the story of the character.",
    "If outcome guidance includes Fumble or Chaos, include an explicit concrete consequence and fail-forward path.",
    "For Fumble specifically, keep the setback tied to the current objective and include one concrete actionable opening in the same scene.",
    "Never derail to unrelated side plots after Fumble; the main objective must stay playable.",
    ...
    "If this is a direct action and resolved direct actions are between 15 and 25, start converging unresolved threads toward a clear endgame beat.",
    "If this is a direct action and resolved direct actions are between 26 and 30, strongly favor decisive progress that sets up immediate scene closure.",
    "If this is a direct action and resolved direct actions exceed 31, prioritize closure-ready outcomes even when the main goal is still unresolved.",
...
export const buildSceneReactionPrompt = (
  ...
  composePrompt(promptTemplates, "scene_controller", [
    ...
    "If action text includes Outcome guidance with Fumble, set failForward=true and keep the next path tied to the current objective.",
    "NPC beat should show NPC agenda pressure, not just mirror player action.",
    "Set shouldIllustrate=true sparingly and only when this turn introduces a distinct location, npc, enemy, or important item worth illustrating.",
    "If shouldIllustrate=true, provide subjectType and a concrete subjectLabel grounded in established fiction.",
    "If no meaningful new visual subject is introduced, set shouldIllustrate=false.",
    ...formatBindingDirectives(input.bindingDirectives),
    `Pitch title: ${input.pitchTitle}`,
    `Pitch description: ${input.pitchDescription}`,
    `Scene intro: ${input.scene.introProse}`,
...
export const buildOutcomeCheckPrompt = (
  ...
  composePrompt(promptTemplates, "outcome_decider", [
    ...
    "Default to shouldCheck=false unless at least one trigger is clearly true from this beat.",
    "Uncertainty alone is not enough; if threat=false and highReward=false, set shouldCheck=false.",
    ...
    "If an Outcome card/check happened in the last 1-2 actions and no immediate threat is present, set shouldCheck=false.",
    ...
    "hardDenyReason: string (max 500 chars; empty when allowHardDenyWithoutOutcomeCheck=false)",
...
export const buildTranscriptIllustrationPromptRequest = (
  promptTemplates: PromptTemplateMap,
  ...
  composePrompt(promptTemplates, "image_generator", [
    ...
    "Avoid dialogue, game terms, markdown, JSON, bullet points, and field labels.",
    "Do not include text overlays, UI elements, logos, or watermarks.",
    input.subjectType ? `Primary subject type: ${input.subjectType}` : "",
    input.subjectLabel ? `Primary subject label: ${input.subjectLabel}` : "",
    `Narrative beat: ${input.narrativeText}`,
    `Scene intro: ${input.scene.introProse}`,
    ...input.scene.orientationBullets.map((bullet) => `Scene cue: ${bullet}`),
    "Recent transcript context:",
    compactTranscript(input.transcriptTail.slice(-8)) || "none",
  ]);
