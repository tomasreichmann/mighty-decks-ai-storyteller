...
export const AdventureModuleEncountersTabPanel = ({
  ...
  const handleCopyEmbed = useCallback(async (encounterSlug: string) => {
    try {
      await copyToClipboard(`<EncounterCard slug="${encounterSlug}" />`);
      setCopiedEncounterSlug(encounterSlug);
      setCopyError(null);
    } catch {
      setCopyError(
        `Could not copy <EncounterCard slug="${encounterSlug}" />.`,
      );
    }
  }, []);
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          {filteredEncounters.map((encounter) => (
            ...
            >
              ...
              >
                <EncounterCardView encounter={encounter} className="max-w-none" />
                <div className="stack gap-1">
                  <Text variant="note" color="steel-dark">
                    {`<EncounterCard slug="${encounter.encounterSlug}" />`}
                  </Text>
                  <Text variant="body" color="iron-light" className="text-sm">
