...
- Imported still images are copied into `AdventureArtifactStore` and rewritten to `/api/adventure-artifacts/...` URLs; the Exiles importer also promotes curated actors as custom actor cards, promotes curated custom asset cards, and rewrites named prose references to canonical `GameCard` embeds while keeping the imported module body markdown-first and free of legacy widget components.
